...
```bash
export platform_ADMIN_ENV=production
export platform_ADMIN_JWT_KEY='replace-with-strong-secret'
export platform_ADMIN_DB_SOURCE='user:password@tcp(mysql:3306)/platform_system?charset=utf8mb4&parseTime=true&loc=Asia%2FShanghai'
export platform_ADMIN_REDIS_ADDR='redis:6379'
export platform_ADMIN_REDIS_PASSWORD='replace-with-redis-password'
export platform_ADMIN_CORS_ORIGINS='https://admin.example.com'
export platform_ADMIN_ENABLE_SWAGGER=false
export platform_ADMIN_DB_DEBUG=false
export platform_ADMIN_DB_MIGRATE=false
```
...
```bash
GOCACHE=/private/tmp/platform-go-cache go test -timeout 60s ./...
```
