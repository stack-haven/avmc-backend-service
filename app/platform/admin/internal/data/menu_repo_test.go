...
func TestMenuRepoRejectsInvalidInputWithoutPanic(t *testing.T) {
	...
	if _, err := repo.Update(context.Background(), &pbCore.Menu{}); !pb.IsMenuInvalidId(err) {
...
func TestMenuRepoRejectsDuplicateNameAndPath(t *testing.T) {
	...
	if _, err := repo.Save(ctx, &pbCore.Menu{Name: "users", Path: &path, Meta: &pbCore.MenuMeta{Title: ptr("Users")}}); err != nil {
		t.Fatalf("save first menu: %v", err)
	}
	if _, err := repo.Save(ctx, &pbCore.Menu{Name: "users", Path: ptr("/other"), Meta: &pbCore.MenuMeta{Title: ptr("Other")}}); !pb.IsMenuNameAlreadyExists(err) {
		t.Fatalf("duplicate name error = %v", err)
	}
	if _, err := repo.Save(ctx, &pbCore.Menu{Name: "other", Path: &path, Meta: &pbCore.MenuMeta{Title: ptr("Other")}}); !pb.IsMenuPathAlreadyExists(err) {
...
func TestMenuRepoRejectsInvalidParentAndCycle(t *testing.T) {
	...
	if _, err := repo.Save(ctx, &pbCore.Menu{Name: "orphan", ParentId: ptr(uint32(999))}); !pb.IsBadRequest(err) {
		t.Fatalf("missing parent error = %v", err)
	}
	parent := client.Menu.Create().SetName("parent").SetTitle("Parent").SaveX(ctx)
	child := client.Menu.Create().SetName("child").SetTitle("Child").SetParentID(parent.ID).SaveX(ctx)
	if _, err := repo.Update(ctx, &pbCore.Menu{Id: parent.ID, Name: "parent", ParentId: &child.ID}); !pb.IsBadRequest(err) {
