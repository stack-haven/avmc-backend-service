...
type TenantQuery struct {
	...
	withPermissionGroupBindings *TenantPermissionGroupQuery
...
// QueryPermissionGroupBindings chains the current query on the "permission_group_bindings" edge.
func (_q *TenantQuery) QueryPermissionGroupBindings() *TenantPermissionGroupQuery {
...
func (_q *TenantQuery) Clone() *TenantQuery {
	...
	return &TenantQuery{
		...
		withPermissionGroupBindings: _q.withPermissionGroupBindings.Clone(),
...
// WithPermissionGroupBindings tells the query-builder to eager-load the nodes that are connected to
// the "permission_group_bindings" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TenantQuery) WithPermissionGroupBindings(opts ...func(*TenantPermissionGroupQuery)) *TenantQuery {
	query := (&TenantPermissionGroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPermissionGroupBindings = query
	return _q
}
...
func (_q *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [1]bool{
			_q.withPermissionGroupBindings != nil,
		}
	)
	...
	if query := _q.withPermissionGroupBindings; query != nil {
		if err := _q.loadPermissionGroupBindings(ctx, query, nodes,
			func(n *Tenant) { n.Edges.PermissionGroupBindings = []*TenantPermissionGroup{} },
			func(n *Tenant, e *TenantPermissionGroup) {
				n.Edges.PermissionGroupBindings = append(n.Edges.PermissionGroupBindings, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *TenantQuery) loadPermissionGroupBindings(ctx context.Context, query *TenantPermissionGroupQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *TenantPermissionGroup)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tenantpermissiongroup.FieldTenantID)
	}
	query.Where(predicate.TenantPermissionGroup(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tenant.PermissionGroupBindingsColumn), fks...))
	}))
