...
func (Role) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Int32("data_scope").Comment("数据范围（1：全部数据权限 2：本人数据权限 3：本部门数据权限 4：本部门及以下数据权限 5：自定部门数据权限）").SchemaType(map[string]string{dialect.MySQL: "tinyint(2)", dialect.Postgres: "tinyint(2)"}).Default(2).Nillable(),
		...
		field.Bool("is_tenant_admin").Default(false).Comment("是否租户管理员角色，仅内部流程维护"),
	}
}
