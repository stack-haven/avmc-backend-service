...
func (r *resourceQuotaRepoStub) ListUsage(_ context.Context, tenantID uint32) ([]*pb.TenantResourceQuotaUsage, error) {
	items := make([]*pb.TenantResourceQuotaUsage, 0, len(r.usages))
	for _, usage := range r.usages {
		if usage.GetTenantId() == tenantID {
			items = append(items, proto.Clone(usage).(*pb.TenantResourceQuotaUsage)) //nolint:errcheck // proto.Clone does not return error
		}
	}
	return items, nil
}

func (r *resourceQuotaRepoStub) GetUsage(_ context.Context, tenantID uint32, resourceKey string) (*pb.TenantResourceQuotaUsage, error) {
	if r.usages == nil {
		r.usages = map[string]*pb.TenantResourceQuotaUsage{}
	}
	if usage := r.usages[resourceKey]; usage != nil {
		return proto.Clone(usage).(*pb.TenantResourceQuotaUsage), nil //nolint:errcheck // proto.Clone does not return error
	}
	return &pb.TenantResourceQuotaUsage{TenantId: tenantID, ResourceKey: resourceKey}, nil
}

func (r *resourceQuotaRepoStub) Consume(_ context.Context, tenantID uint32, resourceKey string, amount, limit int64, unlimited bool, _ string, _ uint32) (*pb.TenantResourceQuotaUsage, bool, error) {
...
func (r *resourceQuotaRepoStub) Release(_ context.Context, tenantID uint32, resourceKey string, amount int64, _ string, _ uint32) (*pb.TenantResourceQuotaUsage, error) {
	usage, _ := r.GetUsage(context.Background(), tenantID, resourceKey)
	usage.Used -= amount
	if usage.Used < 0 {
		usage.Used = 0
...
func TestResourceQuotaUsecaseTreatsMissingLimitAsUnlimited(t *testing.T) {
	...
	uc := NewResourceQuotaUsecase(
		&resourceQuotaRepoStub{},
		&TenantMenuPermissionGroupRepoStub{caps: &pb.GetCurrentTenantCapabilitiesResponse{TenantId: 10}},
		log.NewStdLogger(io.Discard),
	)
