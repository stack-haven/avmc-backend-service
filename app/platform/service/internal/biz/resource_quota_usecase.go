...
type ResourceQuotaRepo interface {
	ListUsage(context.Context, uint32) ([]*pb.TenantResourceQuotaUsage, error)
	GetUsage(context.Context, uint32, string) (*pb.TenantResourceQuotaUsage, error)
	Consume(context.Context, uint32, string, int64, int64, bool, string, uint32) (*pb.TenantResourceQuotaUsage, bool, error)
	Release(context.Context, uint32, string, int64, string, uint32) (*pb.TenantResourceQuotaUsage, error)
}
...
func (uc *ResourceQuotaUsecase) ListCurrent(ctx context.Context) ([]*pb.TenantResourceQuotaUsage, error) {
	...
	byKey := make(map[string]*pb.TenantResourceQuotaUsage, len(rows))
	for _, row := range rows {
		if row != nil && row.GetResourceKey() != "" {
			byKey[row.GetResourceKey()] = row
		}
	}
	result := make([]*pb.TenantResourceQuotaUsage, 0, len(limits)+len(rows))
	for key, limit := range limits {
		usage := byKey[key]
		if usage == nil {
			usage = &pb.TenantResourceQuotaUsage{TenantId: tenantID, ResourceKey: key}
		}
...
func (uc *ResourceQuotaUsecase) CheckCurrent(ctx context.Context, resourceKey string, amount int64) (*pb.CheckCurrentTenantResourceQuotaResponse, error) {
	tenantID, err := currentTenantID(ctx)
	if err != nil {
		return nil, err
	}
	...
	return &pb.CheckCurrentTenantResourceQuotaResponse{
		Allowed: unlimited || usage.GetUsed()+amount <= limit,
		Usage:   usage,
	}, nil
}

func (uc *ResourceQuotaUsecase) ConsumeCurrent(ctx context.Context, resourceKey string, amount int64, idempotencyKey string) (*pb.TenantResourceQuotaUsage, error) {
	tenantID, err := currentTenantID(ctx)
	if err != nil {
		return nil, err
	}
...
func (uc *ResourceQuotaUsecase) ReleaseCurrent(ctx context.Context, resourceKey string, amount int64, idempotencyKey string) (*pb.TenantResourceQuotaUsage, error) {
	tenantID, err := currentTenantID(ctx)
	if err != nil {
		return nil, err
	}
...
func (uc *ResourceQuotaUsecase) ReserveCurrent(ctx context.Context, resourceKey string, amount int64, idempotencyKey string) (*ResourceQuotaReservation, *pb.TenantResourceQuotaUsage, error) {
	idempotencyKey, err := normalizeQuotaIdempotencyKey(idempotencyKey)
	if err != nil {
		return nil, nil, err
	}
...
func (r *ResourceQuotaReservation) Release(ctx context.Context) (*pb.TenantResourceQuotaUsage, error) {
	if r == nil || r.uc == nil {
...
func applyQuotaLimit(usage *pb.TenantResourceQuotaUsage, limit int64, unlimited bool) *pb.TenantResourceQuotaUsage {
	if usage == nil {
		usage = &pb.TenantResourceQuotaUsage{}
	}
	result := proto.Clone(usage).(*pb.TenantResourceQuotaUsage) //nolint:errcheck // proto.Clone does not return error
	if result.Used < 0 {
		result.Used = 0
	}
