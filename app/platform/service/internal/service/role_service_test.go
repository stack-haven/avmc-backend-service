...
func (r *roleRepoServiceStub) Save(_ context.Context, role *pb.Role) (*pb.Role, error) {
	r.saved = role
	return role, nil
}

func (r *roleRepoServiceStub) Update(_ context.Context, role *pb.Role) (*pb.Role, error) {
	r.updated = role
	return role, nil
}

func (r *roleRepoServiceStub) FindByID(context.Context, uint32) (*pb.Role, error) {
	if r.findErr != nil {
	...
	return &pb.Role{Id: 1}, nil
}

func (*roleRepoServiceStub) CountRoles(context.Context, ...listing.Option) (int32, error) {
	return 0, nil
}
func (r *roleRepoServiceStub) ListAll(context.Context) ([]*pb.Role, error) { return r.roles, nil }
func (*roleRepoServiceStub) ListRoles(context.Context, ...listing.Option) ([]*pb.Role, error) {
	return nil, nil
}
...
func protoCloneRole(role *pb.Role) *pb.Role {
	return proto.Clone(role).(*pb.Role) //nolint:errcheck // proto.Clone does not return error
}
...
func TestRoleServiceUpdateRolePreservesMenuIDsWhenMaskExcludesAuthorization(t *testing.T) {
	...
	oldName := "租户管理员"
	newName := "租户运营"
	repo := &roleRepoServiceStub{current: &pb.Role{Id: 7, Name: &oldName, MenuIds: []uint32{1, 2, 3}}}
	service := newRoleServiceForTest(repo)

	if _, err := service.UpdateRole(context.Background(), &pb.UpdateRoleRequest{
		Id:         7,
		Role:       &pb.Role{Name: &newName, MenuIds: []uint32{99}},
...
func TestRoleServiceListRoleSimpleUsesLightweightRoleList(t *testing.T) {
	...
	resp, err := service.ListRoleSimple(context.Background(), &pb.ListRoleSimpleRequest{})
	if err != nil {
...
func TestRoleServiceUpdateRoleAppliesMenuIDsWhenMaskIncludesAuthorization(t *testing.T) {
	...
	name := "租户管理员"
	repo := &roleRepoServiceStub{current: &pb.Role{Id: 7, Name: &name, MenuIds: []uint32{1, 2, 3}}}
	service := newRoleServiceForTest(repo)

	if _, err := service.UpdateRole(context.Background(), &pb.UpdateRoleRequest{
		Id:         7,
		Role:       &pb.Role{MenuIds: []uint32{10, 20}},
...
func TestRoleServiceValidationAndExistByName(t *testing.T) {
	...
	if _, err := service.GetRole(context.Background(), &pb.GetRoleRequest{}); err == nil {
		t.Fatal("GetRole() without id succeeded")
	}
	if _, err := service.CreateRole(context.Background(), &pb.CreateRoleRequest{}); err == nil {
		t.Fatal("CreateRole() without role succeeded")
	}
	if _, err := service.DeleteRole(context.Background(), &pb.DeleteRoleRequest{}); err == nil {
		t.Fatal("DeleteRole() without id succeeded")
	}

	resp, err := service.ExistRoleByName(context.Background(), &pb.ExistRoleByNameRequest{Name: "admin", Id: &roleID})
	if err != nil {
	...
	if _, err := service.UpdateRole(context.Background(), &pb.UpdateRoleRequest{Id: 7, Role: &pb.Role{}}); err != nil {
	...
	if _, err := service.UpdateRole(context.Background(), &pb.UpdateRoleRequest{Id: 7, Role: &pb.Role{}}); !errors.Is(err, expected) {
...
func TestRoleServiceUpdateStatus(t *testing.T) {
	...
	repo := &roleRepoServiceStub{current: &pb.Role{Id: 7, Status: &enabled}}
	service := newRoleServiceForTest(repo)

	if _, err := service.UpdateRoleByStatus(context.Background(), &pb.UpdateRoleByStatusRequest{Id: 7, Status: &disabled}); err != nil {
