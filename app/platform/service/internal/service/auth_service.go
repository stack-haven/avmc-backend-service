...
// 返回值：认证服务实例指针
func NewAuthServiceService(authorizer authzEngine.Authorizer, auc *biz.AuthUsecase, uuc *biz.UserUsecase, logger log.Logger) *AuthServiceService {
...
// IsAuthorized 执行授权检查。
// 请求字段与 authz 语义的映射（与 app/platform/service/client 的鉴权委托客户端一致）：
//   - resource → 对象（操作/资源路径）
//   - action   → 操作（HTTP 方法或 gRPC 方法名）
//
// 注意：主体（sub）与租户（tenant）不信任请求参数，改为从认证后的 JWT claims 提取，
// 防止产品服务被攻破后伪造身份；resource/action 描述的是访问目标，仍由委托方传入。
//
// 返回：无错误表示允许；错误（FORBIDDEN）表示拒绝。
func (s *AuthServiceService) IsAuthorized(ctx context.Context, req *pb.IsAuthorizedRequest) (*pb.IsAuthorizedResponse, error) {
	if req == nil {
		return nil, kerrors.BadRequest("AUTHZ_REQUEST_REQUIRED", "authorization request is required")
	}
	// 身份从认证后的 JWT claims 提取（AuthnMiddleware 已验证并注入）。
	claims, ok := authn.AuthClaimsFromContext(ctx)
	if !ok || claims == nil {
		return nil, kerrors.Unauthorized("AUTHZ_UNAUTHENTICATED", "missing authenticated identity")
	}
	sub := authzEngine.Subject(claims.GetSubject())
	tenant := authzEngine.Tenant(claims.GetTenant())
	// 资源与动作描述访问目标，由委托方传入。
