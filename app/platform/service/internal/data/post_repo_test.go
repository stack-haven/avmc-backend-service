...
func TestPostRepoListReturnsPersistedFields(t *testing.T) {
	...
	created, err := repo.Save(ctx, &pb.Post{
		Name:   ptr("developer"),
		Sort:   ptr(int32(7)),
...
func TestPostRepoReturnsTypedBusinessErrors(t *testing.T) {
	...
	if _, err := repo.Save(ctx, &pb.Post{}); !pb.IsPostNameCannotBeEmpty(err) {
...
func TestPostRepoSavePropagatesUniquenessQueryError(t *testing.T) {
	...
	_, err := repo.Save(tenantContext(1), &pb.Post{Name: ptr("developer")})
