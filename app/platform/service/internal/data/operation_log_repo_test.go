...
func TestOperationLogRepoAppliesRoleDataScope(t *testing.T) {
	...
	if err := repo.Append(ctx, &pb.OperationLog{OperatorId: &actor.ID, Module: "user", Action: "create"}); err != nil {
		t.Fatalf("append actor log: %v", err)
	}
	if err := repo.Append(ctx, &pb.OperationLog{OperatorId: &other.ID, Module: "user", Action: "delete"}); err != nil {
		t.Fatalf("append other log: %v", err)
	}
	if err := repo.Append(ctx, &pb.OperationLog{Module: "system", Action: "sync"}); err != nil {
		t.Fatalf("append system log: %v", err)
	}

	actorCtx := tenantUserContext(1, actor.ID)
	items, total, err := repo.List(actorCtx, &pb.ListOperationLogsRequest{})
	if err != nil {
	...
	items, total, err = repo.List(adminCtx, &pb.ListOperationLogsRequest{})
	if err != nil {
