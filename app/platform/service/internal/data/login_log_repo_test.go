...
func TestLoginLogRepoTenantIsolationAndFiltering(t *testing.T) {
	...
	success := "success"
	failure := "failure"
	if err := repo.Append(tenantContext(1), &pb.LoginLog{
	...
	if err := repo.Append(tenantContext(2), &pb.LoginLog{
		TenantId: 2, UserId: &adminTwo.ID, Identity: "admin", LoginType: "username", Result: success,
	}); err != nil {
		t.Fatalf("append tenant 2 success: %v", err)
	}

	items, total, err := repo.List(tenantUserContext(1, adminOne.ID), &pb.ListLoginLogsRequest{Result: &success})
	if err != nil {
	...
	items, total, err = repo.List(tenantUserContext(2, adminTwo.ID), &pb.ListLoginLogsRequest{})
	if err != nil {
...
func TestLoginLogRepoAppliesRoleDataScope(t *testing.T) {
	...
	success := "success"
	if err := repo.Append(ctx, &pb.LoginLog{UserId: &actor.ID, Identity: "actor", LoginType: "username", Result: success}); err != nil {
		t.Fatalf("append actor log: %v", err)
	}
	if err := repo.Append(ctx, &pb.LoginLog{UserId: &other.ID, Identity: "other", LoginType: "username", Result: success}); err != nil {
		t.Fatalf("append other log: %v", err)
	}
	if err := repo.Append(ctx, &pb.LoginLog{Identity: "anonymous", LoginType: "username", Result: success}); err != nil {
		t.Fatalf("append anonymous log: %v", err)
	}

	items, total, err := repo.List(tenantUserContext(1, actor.ID), &pb.ListLoginLogsRequest{})
	if err != nil {
	...
	items, total, err = repo.List(tenantUserContext(1, admin.ID), &pb.ListLoginLogsRequest{})
	if err != nil {
