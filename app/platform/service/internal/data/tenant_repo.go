...
func (r *tenantRepo) entToProto(e *gen.Tenant) *pb.Tenant {
	if e == nil {
		return nil
	}
	return &pb.Tenant{
		...
		LifecycleStatus: pb.TenantLifecycleStatus(e.LifecycleStatus).Enum(),
...
func tenantAdminToProto(e *gen.User) *pb.User {
	if e == nil {
		return nil
	}
...
func (r *tenantRepo) ListAdmins(ctx context.Context, tenantID uint32) ([]*pb.User, error) {
	...
	result := make([]*pb.User, 0, len(items))
...
func (r *tenantRepo) UpdateAdmin(ctx context.Context, tenantID, adminUserID uint32, realname, email, phone *string) (*pb.User, error) {
	systemCtx := entviewer.NewSystemContext(ctx)
...
func (r *tenantRepo) Provision(ctx context.Context, input *biz.TenantProvisioning) (*biz.TenantProvisioningResult, error) {
	...
	tenantRow, err := tx.Tenant.Create().
		...
		SetLifecycleStatus(int32(pb.TenantLifecycleStatus_TENANT_LIFECYCLE_STATUS_PENDING)).
		SetIsPlatform(false).
	...
	if _, err = tx.Tenant.UpdateOneID(tenantRow.ID).
		SetLifecycleStatus(int32(pb.TenantLifecycleStatus_TENANT_LIFECYCLE_STATUS_ACTIVE)).
		SetActivatedAt(now).
...
func (r *tenantRepo) Update(ctx context.Context, g *pb.Tenant, operatorID uint32) (*pb.Tenant, error) {
...
func (r *tenantRepo) FindByID(ctx context.Context, id uint32) (*pb.Tenant, error) {
...
func (r *tenantRepo) ListTenants(ctx context.Context, opts ...listing.Option) ([]*pb.Tenant, error) {
...
func (r *tenantRepo) UpdateLifecycle(ctx context.Context, id uint32, lifecycle pb.TenantLifecycleStatus) (*pb.Tenant, error) {
	now := time.Now()
	...
	case pb.TenantLifecycleStatus_TENANT_LIFECYCLE_STATUS_CANCELLED:
...
func (r *tenantRepo) tenantGroups(e *gen.Tenant) []*pb.TenantMenuPermissionGroup {
	if e == nil || len(e.Edges.Groups) == 0 {
		return nil
	}
	groups := make([]*pb.TenantMenuPermissionGroup, 0, len(e.Edges.Groups))
