...
func buildMenuTree(menus []*pb.Menu) []*pb.Menu {
	nodes := make(map[uint32]*pb.Menu, len(menus))
	roots := make([]*pb.Menu, 0, len(menus))
	for _, m := range menus {
		if m == nil || m.GetType() == pb.MenuType_MENU_TYPE_BUTTON {
			continue
		}
		m.Children = nil
		nodes[m.GetId()] = m
