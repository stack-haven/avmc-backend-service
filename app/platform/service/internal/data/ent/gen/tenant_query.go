...
type TenantQuery struct {
	...
	withGroups *TenantMenuPermissionGroupQuery
...
func (_q *TenantQuery) QueryGroups() *TenantMenuPermissionGroupQuery {
...
func (_q *TenantQuery) WithGroups(opts ...func(*TenantMenuPermissionGroupQuery)) *TenantQuery {
	query := (&TenantMenuPermissionGroupClient{config: _q.config}).Query()
...
func (_q *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := _q.withGroups; query != nil {
		if err := _q.loadGroups(ctx, query, nodes,
			func(n *Tenant) { n.Edges.Groups = []*TenantMenuPermissionGroup{} },
			func(n *Tenant, e *TenantMenuPermissionGroup) { n.Edges.Groups = append(n.Edges.Groups, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TenantQuery) loadGroups(ctx context.Context, query *TenantMenuPermissionGroupQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *TenantMenuPermissionGroup)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint32]*Tenant)
	nids := make(map[uint32]map[*Tenant]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tenant.GroupsTable)
		s.Join(joinT).On(s.C(tenantmenupermissiongroup.FieldID), joinT.C(tenant.GroupsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(tenant.GroupsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tenant.GroupsPrimaryKey[0]))
