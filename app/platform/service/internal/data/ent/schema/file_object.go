...
func (FileObject) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("upload_id").Comment("分片上传会话ID（本地=临时目录路径，对象存储预留为 multipart upload id）").MaxLen(255).Default(""),
		field.Int64("part_size").Comment("分片大小（字节），0 表示非分片上传").Min(0).Default(0),
		field.Int32("total_parts").Comment("总分片数，0 表示非分片上传").Default(0),
