...
func TestRoleRepoSaveAndUpdateMenuIDs(t *testing.T) {
	...
	role, err := repo.Save(ctx, &pb.Role{
		Name:    ptr("admin"),
...
func TestRoleRepoEnforcesTenantIsolation(t *testing.T) {
	...
	repo := NewRoleRepo(&Data{db: client}, log.NewStdLogger(io.Discard))
	first, err := repo.Save(tenantContext(1), &pb.Role{Name: ptr("operator")})
	if err != nil {
		t.Fatalf("save tenant one role: %v", err)
	}
	if _, err := repo.Save(tenantContext(2), &pb.Role{Name: ptr("operator")}); err != nil {
...
func TestRoleRepoRejectsInvalidMenuIDs(t *testing.T) {
	...
	if _, err := repo.Save(tenantContext(1), &pb.Role{
		Name:    ptr("operator"),
		MenuIds: []uint32{999},
...
func TestRoleRepoListReturnsMenuIDs(t *testing.T) {
	...
	repo := NewRoleRepo(&Data{db: client}, log.NewStdLogger(io.Discard))
	if _, err := repo.Save(ctx, &pb.Role{Name: ptr("listed-role"), MenuIds: []uint32{menuItem.ID}}); err != nil {
		t.Fatalf("save role: %v", err)
	}
...
func TestRoleRepoExcludesDeletedMenusFromAuthorizations(t *testing.T) {
	...
	role, err := roleRepo.Save(ctx, &pb.Role{Name: ptr("menu-cleanup-role"), MenuIds: []uint32{menuItem.ID}})
	if err != nil {
		t.Fatalf("save role: %v", err)
	}
...
func TestRoleRepoProtectsTenantAdminRole(t *testing.T) {
	...
	if _, err := repo.Update(ctx, &pb.Role{
		Id:     adminRole.ID,
