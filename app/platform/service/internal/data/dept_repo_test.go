...
func TestDeptRepoDerivesHierarchyAndRejectsCrossTenantReferences(t *testing.T) {
	...
	parent, err := repo.Save(tenantOne, &pb.Dept{Name: ptr("parent")})
	if err != nil {
		t.Fatalf("save parent: %v", err)
	}
	child, err := repo.Save(tenantOne, &pb.Dept{
		Name:      ptr("child"),
	...
	if _, err := repo.Save(tenantTwo, &pb.Dept{
...
func TestDeptRepoRejectsHierarchyCycle(t *testing.T) {
	...
	repo := NewDeptRepo(&Data{db: client}, log.NewStdLogger(io.Discard))
	ctx := tenantContext(1)
	root, err := repo.Save(ctx, &pb.Dept{Name: ptr("root")})
	if err != nil {
		t.Fatalf("save root: %v", err)
	}
	parent, err := repo.Save(ctx, &pb.Dept{Name: ptr("parent"), ParentId: ptr(root.GetId())})
	if err != nil {
		t.Fatalf("save parent: %v", err)
	}
	child, err := repo.Save(ctx, &pb.Dept{Name: ptr("child"), ParentId: ptr(parent.GetId())})
	if err != nil {
		t.Fatalf("save child: %v", err)
	}

	if _, err := repo.Update(ctx, &pb.Dept{
		Id:       parent.GetId(),
		Name:     ptr("parent"),
...
func TestDeptRepoProtectsChildrenOnDelete(t *testing.T) {
	...
	if _, err := repo.Save(ctx, &pb.Dept{Name: ptr("child"), ParentId: ptr(parent.GetId())}); err != nil {
		t.Fatalf("save child: %v", err)
	}
...
func TestDeptRepoProtectsUserAndRoleReferencesOnDelete(t *testing.T) {
	...
	repo := NewDeptRepo(&Data{db: client}, log.NewStdLogger(io.Discard))
	ctx := tenantContext(1)
	userDept, _ := repo.Save(ctx, &pb.Dept{Name: ptr("user-dept")})
	roleDept, _ := repo.Save(ctx, &pb.Dept{Name: ptr("role-dept")})
	client.User.Create().
		SetName("dept-user").
...
func TestDeptRepoMoveRewritesDescendantAncestors(t *testing.T) {
	...
	repo := NewDeptRepo(&Data{db: client}, log.NewStdLogger(io.Discard))
	ctx := tenantContext(1)
	rootA, _ := repo.Save(ctx, &pb.Dept{Name: ptr("root-a")})
	rootB, _ := repo.Save(ctx, &pb.Dept{Name: ptr("root-b")})
	child, _ := repo.Save(ctx, &pb.Dept{Name: ptr("child"), ParentId: ptr(rootA.GetId())})
	grandchild, _ := repo.Save(ctx, &pb.Dept{Name: ptr("grandchild"), ParentId: ptr(child.GetId())})
...
func TestDeptRepoListReturnsAncestors(t *testing.T) {
	...
	repo := NewDeptRepo(&Data{db: client}, log.NewStdLogger(io.Discard))
	ctx := tenantContext(1)
	parent, err := repo.Save(ctx, &pb.Dept{Name: ptr("parent")})
	if err != nil {
		t.Fatalf("save parent: %v", err)
	}
	if _, err := repo.Save(ctx, &pb.Dept{Name: ptr("child"), ParentId: ptr(parent.GetId())}); err != nil {
...
func TestDeptRepoTransferAndDeleteMovesUsersAtomically(t *testing.T) {
	...
	repo := NewDeptRepo(&Data{db: client}, log.NewStdLogger(io.Discard))
	ctx := tenantContext(1)
	root, _ := repo.Save(ctx, &pb.Dept{Name: ptr("root")})
	source, _ := repo.Save(ctx, &pb.Dept{Name: ptr("source"), ParentId: ptr(root.GetId())})
	target, _ := repo.Save(ctx, &pb.Dept{Name: ptr("target"), ParentId: ptr(root.GetId())})
...
func TestDeptRepoTransferAndDeleteRejectsChildrenAndCrossTenantTarget(t *testing.T) {
	...
	root, _ := repo.Save(ctx, &pb.Dept{Name: ptr("root")})
	source, _ := repo.Save(ctx, &pb.Dept{Name: ptr("source"), ParentId: ptr(root.GetId())})
	if _, err := repo.Save(ctx, &pb.Dept{Name: ptr("child"), ParentId: ptr(source.GetId())}); err != nil {
		t.Fatal(err)
	}
	targetTwo, _ := repo.Save(tenantContext(2), &pb.Dept{Name: ptr("tenant-two-root")})

	if _, err := repo.TransferAndDelete(ctx, source.GetId(), root.GetId()); !pb.IsDeptCannotDeleteWithChildren(err) {
		t.Fatalf("children error = %v", err)
	}
	leaf, _ := repo.Save(ctx, &pb.Dept{Name: ptr("leaf"), ParentId: ptr(root.GetId())})
