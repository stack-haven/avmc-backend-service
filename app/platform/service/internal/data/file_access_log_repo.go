...
func fileAccessLogToProto(row *gen.FileAccessLog) *pb.FileAccessLog {
	if row == nil {
		return nil
	}
	return &pb.FileAccessLog{
		Id:           row.ID,
...
func (r *fileAccessLogRepo) Append(ctx context.Context, value *pb.FileAccessLog) error {
...
func (r *fileAccessLogRepo) List(ctx context.Context, opts ...listing.Option) ([]*pb.FileAccessLog, error) {
	if _, err := r.RequireTenantID(ctx); err != nil {
		return nil, err
	}
