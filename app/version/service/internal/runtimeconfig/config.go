...
func ApplyEnvOverrides(bc *conf.Bootstrap) {
	...
	if bc.Server != nil {
		if bc.Server.Http != nil {
			if v := strings.TrimSpace(os.Getenv("AVMC_VERSION_HTTP_ADDR")); v != "" {
				bc.Server.Http.Addr = v
			}
		}
		if bc.Server.Grpc != nil {
			if v := strings.TrimSpace(os.Getenv("AVMC_VERSION_GRPC_ADDR")); v != "" {
	...
	if bc.Data != nil {
		if bc.Data.Database != nil {
			if v := strings.TrimSpace(os.Getenv("AVMC_VERSION_DB_DRIVER")); v != "" {
				bc.Data.Database.Driver = v
			}
			if v := strings.TrimSpace(os.Getenv("AVMC_VERSION_DB_SOURCE")); v != "" {
				bc.Data.Database.Source = v
			}
			if v, ok := envBool("AVMC_VERSION_DB_DEBUG"); ok {
				bc.Data.Database.Debug = v
			}
			if v, ok := envBool("AVMC_VERSION_DB_MIGRATE"); ok {
				bc.Data.Database.Migrate = v
			}
		}
		if bc.Data.Redis != nil {
			if v := strings.TrimSpace(os.Getenv("AVMC_VERSION_REDIS_ADDR")); v != "" {
				bc.Data.Redis.Addr = v
			}
			if v, ok := os.LookupEnv("AVMC_VERSION_REDIS_PASSWORD"); ok {
...
func Validate(bc *conf.Bootstrap) error {
	...
	if bc.Server.Http == nil || strings.TrimSpace(bc.Server.Http.Addr) == "" {
		return ConfigError("AVMC_VERSION_HTTP_ADDR must not be empty")
	}
	if bc.Server.Grpc == nil || strings.TrimSpace(bc.Server.Grpc.Addr) == "" {
		return ConfigError("AVMC_VERSION_GRPC_ADDR must not be empty")
	}
	if bc.Data == nil {
	...
	if strings.TrimSpace(bc.Data.Database.Driver) == "" {
		return ConfigError("AVMC_VERSION_DB_DRIVER must not be empty")
	}
	source := strings.TrimSpace(bc.Data.Database.Source)
	if source == "" {
		return ConfigError("AVMC_VERSION_DB_SOURCE must not be empty")
	}
	if bc.Data.Redis == nil || strings.TrimSpace(bc.Data.Redis.Addr) == "" {
		return ConfigError("AVMC_VERSION_REDIS_ADDR must not be empty")
	}
	if IsProduction() {
		if bc.Data.Database.Debug {
			return ConfigError("AVMC_VERSION_DB_DEBUG must be false in production")
		}
		if bc.Data.Database.Migrate {
			return ConfigError("AVMC_VERSION_DB_MIGRATE must be false in production; run migrations out of band")
		}
		if unsafeProductionDatabaseSource(source) {
			return ConfigError("AVMC_VERSION_DB_SOURCE must not use root credentials or placeholders in production")
		}
		if strings.TrimSpace(bc.Data.Redis.Password) == "" || strings.Contains(strings.ToLower(bc.Data.Redis.Password), "replace-me") {
			return ConfigError("AVMC_VERSION_REDIS_PASSWORD must not be empty or a placeholder in production")
		}
	}
	return nil
}
...
func IsProduction() bool {
	for _, key := range []string{"AVMC_VERSION_ENV", "DEPLOY_ENV", "APP_ENV"} {
