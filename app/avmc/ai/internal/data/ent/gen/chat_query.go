...
type ChatQuery struct {
	...
	withNamedChatMessages map[string]*ChatMessageQuery
...
func (_q *ChatQuery) QueryChatMessages() *ChatMessageQuery {
...
func (_q *ChatQuery) FirstID(ctx context.Context) (id uint32, err error) {
	var ids []uint32
...
func (_q *ChatQuery) FirstIDX(ctx context.Context) uint32 {
...
func (_q *ChatQuery) OnlyID(ctx context.Context) (id uint32, err error) {
	var ids []uint32
...
func (_q *ChatQuery) OnlyIDX(ctx context.Context) uint32 {
...
func (_q *ChatQuery) IDs(ctx context.Context) (ids []uint32, err error) {
...
func (_q *ChatQuery) IDsX(ctx context.Context) []uint32 {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *ChatQuery) WithChatMessages(opts ...func(*ChatMessageQuery)) *ChatQuery {
	query := (&ChatMessageClient{config: _q.config}).Query()
...
func (_q *ChatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chat, error) {
	...
	if query := _q.withChatMessages; query != nil {
		if err := _q.loadChatMessages(ctx, query, nodes,
			func(n *Chat) { n.Edges.ChatMessages = []*ChatMessage{} },
			func(n *Chat, e *ChatMessage) { n.Edges.ChatMessages = append(n.Edges.ChatMessages, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedChatMessages {
		if err := _q.loadChatMessages(ctx, query, nodes,
			func(n *Chat) { n.appendNamedChatMessages(name) },
			func(n *Chat, e *ChatMessage) { n.appendNamedChatMessages(name, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ChatQuery) loadChatMessages(ctx context.Context, query *ChatMessageQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *ChatMessage)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint32]*Chat)
	for i := range nodes {
	...
	query.Where(predicate.ChatMessage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(chat.ChatMessagesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.chat_chat_messages
		if fk == nil {
			return fmt.Errorf(`foreign-key "chat_chat_messages" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "chat_chat_messages" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *ChatQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(chat.Table, chat.Columns, sqlgraph.NewFieldSpec(chat.FieldID, field.TypeUint32))
	_spec.From = _q.sql
...
// WithNamedChatMessages tells the query-builder to eager-load the nodes that are connected to the "chat_messages"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *ChatQuery) WithNamedChatMessages(name string, opts ...func(*ChatMessageQuery)) *ChatQuery {
	query := (&ChatMessageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedChatMessages == nil {
		_q.withNamedChatMessages = make(map[string]*ChatMessageQuery)
	}
	_q.withNamedChatMessages[name] = query
	return _q
}
