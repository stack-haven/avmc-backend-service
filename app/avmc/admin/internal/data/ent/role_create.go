...
func (_c *RoleCreate) check() error {
	...
	if _, ok := _c.mutation.DomainID(); !ok {
		return &ValidationError{Name: "domain_id", err: errors.New(`ent: missing required field "Role.domain_id"`)}
	}
	if v, ok := _c.mutation.DomainID(); ok {
		if err := role.DomainIDValidator(v); err != nil {
			return &ValidationError{Name: "domain_id", err: fmt.Errorf(`ent: validator failed for field "Role.domain_id": %w`, err)}
		}
	}
	if _, ok := _c.mutation.Name(); !ok {
